...
import { ExternalLink, Building2, Users, Layers, Briefcase, Handshake, ShieldCheck } from "lucide-react";
import { Agency, Client, CaseStudy } from "@/types/agency";
                ...
                <div className="bg-slate-50 border-b p-6 sticky top-0 z-10">
                    <DialogHeader>
                        ...
                        <DialogDescription className="sr-only">
                            Agency profile for {partner.name}
                        </DialogDescription>
                        <div className="flex flex-col gap-1 mt-1">
                            ...
                            <div className="flex gap-2 mt-1">
                                {partner.is_group_member && (
                                    <Badge variant="outline" className="bg-purple-50 text-purple-700 border-purple-200 flex items-center gap-1 w-fit">
                                        <ShieldCheck className="h-3 w-3" />
                                        {partner.parent_company && partner.parent_company !== "Self (Group Head)"
                                            ? `Part of ${partner.parent_company}`
                                            : "Agency Group Head"}
                                    </Badge>
                                )}
                                {partner.revenue_estimate && (
                                    <Badge variant="secondary" className="bg-emerald-50 text-emerald-700 border-emerald-100 w-fit">
                                        Est. {partner.revenue_estimate}
                                    </Badge>
                                )}
                            </div>
                        </div>
                ...
                <div className="p-6 space-y-8">
                    ...
                    <section>
                        <div className="flex justify-between items-start mb-3">
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-500 uppercase tracking-wide">
                            ...
                            <div className="flex gap-4">
                                {partner.headcount && (
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                        ...
                                        <span>{partner.headcount} employees</span>
                                    </div>
                                )}
                                {partner.office_locations && partner.office_locations.length > 0 && (
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500">
                                        <Building2 className="h-3.5 w-3.5 text-slate-400" />
                                        <span>{partner.office_locations[0]}</span>
                                    </div>
                                )}
                                {partner.is_group_member && partner.parent_company && (
                                    <div className="flex items-center gap-1.5 text-xs text-purple-600 font-medium">
                                        <ShieldCheck className="h-3.5 w-3.5" />
                                        <span>Group: {partner.parent_company}</span>
                                    </div>
                                )}
                            </div>
                        </div>
                    ...
                    <section>
                        ...
                        <div className="flex flex-wrap gap-2">
                            {clients.slice(0, 15).map((client: Client, idx: number) => (
                                <div key={idx} className="flex items-center gap-2 bg-slate-100 px-3 py-1.5 rounded-full border border-slate-200 text-xs">
                    ...
                    {/* TECHNOLOGY STACK */}
                    {partner.tech_stack && partner.tech_stack.length > 0 && (
                        <section>
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">
                                <Layers className="h-4 w-4" /> Technology Stack
                            </h3>
                            <div className="flex flex-wrap gap-2">
                                {partner.tech_stack.map((tech, idx) => (
                                    <Badge key={idx} variant="outline" className="border-blue-200 text-blue-700 bg-blue-50/50">
                                        {tech}
                                    </Badge>
                                ))}
                            </div>
                    ...
                    <section>
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            {caseStudies.length > 0 ? caseStudies.map((study: CaseStudy, idx: number) => (
                                <a
                                    key={idx}
                                    href={study.url || '#'}
                                    target="_blank"
                                ...
                                >
                                    <span className="font-medium text-slate-800 group-hover:text-blue-700 truncate mr-2">
                                        {study.title || "Untitled Case Study"}
                                    </span>
                                    <ExternalLink className="h-4 w-4 text-slate-400 group-hover:text-blue-500 shrink-0" />
                                </a>
                            )) : <p className="text-slate-400 italic col-span-2">No case studies available</p>}
                        </div>
                    </section>
                    ...
                    {partners.length > 0 && (
                        <section className="pt-4 border-t border-slate-100">
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">
                                <Building2 className="h-4 w-4" /> Notable Partners
                            </h3>
                                    ...
                                    <div key={idx} className="p-3 bg-white border border-slate-200 rounded-lg hover:shadow-sm transition-shadow">
                                        ...
                                        <div className="flex gap-2">
                                            {director.linkedin_url && (
                                                <a href={director.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 text-xs flex items-center gap-1">
                    ...
                    {/* SIBLING AGENCIES (Recursive Discovery) */}
                    {partner.sibling_agencies && partner.sibling_agencies.length > 0 && (
                        <section className="pt-4 border-t border-slate-100">
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-purple-600 uppercase tracking-wide mb-3">
                                <ShieldCheck className="h-4 w-4" /> Sibling Agencies (Same Group)
                            </h3>
                            <div className="flex flex-wrap gap-2">
                                {partner.sibling_agencies.map((sibling, idx) => (
                            ...
                            <p className="text-[10px] text-slate-400 mt-2 italic">
                                * These agencies were automatically discovered as part of the {partner.parent_company} group.
                            </p>
                        </section>
                    )}
                    {partner.partner_managers && partner.partner_managers.length > 0 && (
                        <section className="pt-4 border-t border-slate-100">
                            <h3 className="flex items-center gap-2 text-sm font-semibold text-slate-500 uppercase tracking-wide mb-3">
                                <Handshake className="h-4 w-4" /> Partner Managers
                            </h3>
                            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                                {partner.partner_managers.map((manager, idx) => (
                                    <div key={`pm-${idx}`} className="p-3 bg-white border border-blue-100 rounded-lg shadow-sm hover:shadow transition-shadow">
                                        <div className="font-medium text-slate-900 truncate" title={manager.name}>{manager.name}</div>
                                        <div className="text-xs text-blue-600 font-medium truncate mb-2" title={manager.role}>{manager.role}</div>
                                        <div className="flex gap-2">
                                            {manager.linkedin_url && (
                                                <a href={manager.linkedin_url} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:text-blue-800 text-xs flex items-center gap-1">
                                                    <Briefcase className="h-3 w-3" /> LinkedIn
                                                </a>
                                            )}
                                            {manager.email && (
                                                <a href={`mailto:${manager.email}`} className="text-slate-600 hover:text-slate-900 text-xs flex items-center gap-1">
                                                    📧 Email
                                                </a>
                                            )}
                                        </div>
